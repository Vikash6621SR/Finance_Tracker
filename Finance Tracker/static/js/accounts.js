...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const cashAccounts = document.getElementById("cashAccounts");
  ...
  if (typeof apiGet !== "function") {
    console.error("accounts.js: apiGet() was not found.");
    return;
  }
  ...
  function setupEvents() {
    ...
    typeFilter?.addEventListener("change", renderAccounts);
  ...
  async function loadAccounts() {
    ...
    try {
      ...
      accounts = extractArray(response, ["accounts", "data", "content"])
  ...
  function normalizeAccount(account) {
    ...
    return {
      ...
      name: account.name ?? account.accountName ?? account.title ?? "Account",

      type:
        account.type ?? account.accountType ?? account.account_type ?? "OTHER",
  ...
  function updateSummary() {
    ...
    if (bankAccounts) {
      bankAccounts.textContent = accounts.filter(function (account) {
        return normalizeType(account.type) === "BANK";
      }).length;
    }

    if (cashAccounts) {
      cashAccounts.textContent = accounts.filter(function (account) {
        return normalizeType(account.type) === "CASH";
      }).length;
    }
  }
  ...
  function renderAccounts() {
    ...
    const filtered = accounts.filter(function (account) {
      const matchesSearch =
        ...
        String(account.description).toLowerCase().includes(search);

      const matchesType =
        type === "all" || normalizeType(account.type) === type;
    ...
    if (accounts.length === 0) {
      ...
      if (resultText) {
        resultText.textContent = "0 accounts";
      }
    ...
    if (filtered.length === 0) {
      ...
      if (heading) {
        heading.textContent = "No matching accounts";
      }
      ...
      if (resultText) {
        resultText.textContent = "0 matching accounts";
      }
    ...
    if (heading) {
      heading.textContent = "No accounts found";
    }
  ...
  function createAccountCard(account) {
    ...
    card.innerHTML = `
            ...
            <p class="account-description">
                ${escapeHtml(account.description || "No description added.")}
            </p>
  ...
  function formatType(type) {
    ...
    return (
      names[value] ||
      value.replace(/_/g, " ").replace(/\b\w/g, function (letter) {
  ...
  function openEditModal(account) {
    ...
    if (accountType) {
      accountType.value = normalizeType(account.type);
    }

    if (accountBalance) {
      accountBalance.value = Number(account.balance) || 0;
    }
  ...
  async function handleSubmit(event) {
    ...
    const type = normalizeType(accountType?.value);

    const balance = Number(accountBalance?.value);

    const description = accountDescription?.value.trim() || "";

    if (!name) {
      showModalMessage("Please enter an account name.");
    ...
    if (!accountType?.value) {
      showModalMessage("Please select an account type.");
    ...
    if (!Number.isFinite(balance) || balance < 0) {
      showModalMessage("Please enter a valid opening balance.");
    ...
    try {
      ...
      if (saveButton) {
        ...
        saveButton.textContent = editingId ? "Update Account" : "Save Account";
      }
    }
  }
  ...
  async function updateAccount(id, payload) {
    ...
    return apiPut("/accounts/" + encodedId, payload);
  }
  ...
  async function handleDelete() {
    ...
    try {
      await apiDelete("/accounts/" + encodeURIComponent(String(id)));
  ...
  function showLoadError(error) {
    ...
    if (resultText) {
      resultText.textContent = "Unable to load accounts";
    }
  }
    ...
    try {
      if (typeof logoutUser === "function") {
        ...
        try {
          ...
          console.warn("Logout API unavailable.", error);
        }
      }
    } finally {
